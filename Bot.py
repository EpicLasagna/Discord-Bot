...
async def _8ballerror(ctx, error):
    if isinstance(error, commands.MissingRequiredArgument):
        embed = discord.Embed(title=":x: Missing Parameters!")
...
async def whois_error(ctx, error):
    if isinstance(error, commands.MemberNotFound):
        embed = discord.Embed(title=":x: That is not a member of this server!")
        await ctx.send(embed=embed)
...
newrules = [":one: Don't disrespect anyone", ":two: I made this for a test"]

@client.command()
async def rule(ctx, *, number):
    await ctx.send(newrules[int(number)-1])

@client.command()
async def rules(ctx):
    for rulenums in newrules:
        await ctx.send(rulenums)

@client.command()
async def embed_maker(ctx, newtitle, * , newdesc):
    embed = discord.Embed(title=f"{newtitle}", description=f"{newdesc}", color=0x32a6a8)
    embed.set_footer(icon_url=ctx.author.avatar_url ,text= f"Requested by: {ctx.author.name}")
    await ctx.send(embed=embed)
    await ctx.send("Embed has been successfully created! :white_check_mark:")

@embed_maker.error
async def embed_maker_error(ctx, error):
    if isinstance(error, commands.MissingRequiredArgument):
        embed = discord.Embed(title=":x: Missing Parameters!",description=None)
        await ctx.send(embed=embed)
